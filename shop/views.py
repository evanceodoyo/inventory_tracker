...
def home(request):
    try:
        ...
        if category := request.GET.get('category'):
            products = Product.objects.filter(categories__slug__in=[category], status=True)
        else:
        ...
        return render(request, 'products.html', {'page_title':"Shop", "products":products, "categories":categories})
...
def product_detail(request, product_slug):
    ...
    related_products = Product.objects.filter(status=True).filter(categories__in=related_product_ids).exclude(id=product.id).distinct()

    return render(request, 'product.html', {"page_title": product.name, "product": product, "categories":categories, "related_products": related_products})
